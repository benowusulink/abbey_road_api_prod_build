...
const store_pdf_documents = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    if (file.fieldname === "who_we_are_pdf") {
      cb(
        null,
        path.join(__dirname, "..", "public", "documentation", "index-page"),
      );
    }
  },
  ...
  sqlite
    ...
    .then((elements) => {
      if (
        ...
        const estate_element_text = elements
          .filter((estate_element) => {
            if (
              estate_element.section === "estate_elements" &&
  ...
  sqlite
    ...
    .then((result) => {
      if (result.events_table.length > 0) {
        const top3_events = result.events_table
          .sort((a, b) => {
      ...
      if (result.news_table.length > 0) {
        const top3_news = result.news_table
          .sort((a, b) => {
...
app.post(
  "/api/admin_board_members_page/add_board_documents",
  upload_pdf.single("board_members_pdf"),
  (req, res) => {
...
app.put(
  "/api/admin_home_page/edit_who_we_are_documents",
  upload_pdf.single("who_we_are_pdf"),
  (req, res) => {
    // error uploading file or no file from client
    if (!req.file) {
      console.log("error uploading file or no file to upload");
...
app.put(
  "/api/admin_home_page/edit_our_estates_images",
  upload_images.single("our_estates_image"),
  (req, res) => {
...
app.put(
  "/api/admin_home_page/edit_our_service_images",
  upload_images.single("service_image"),
  (req, res) => {
    // error uploading image or no file from client
    if (!req.file) {
      console.log(" no image to upload");
...
app.put(
  "/api/admin_estate_page/edit_selected_estate",
  upload_images.single("edited_image"),
  (req, res) => {
    ...
    // success uploading file & file from client
      ...
      fs.rm(
        path.join(
          ...
          "images",
          "estate-page",
          `${JSON.parse(req.body.image_name)}`,
        ),
        (err) => {
          if (err) {
            console.log("couldnt delete img", err);
            ...
            sqlite
              .transaction((trx) => {
                return trx("estate_page_images")
                  ...
                  .then((data1) => {
                    if (
                      Object.keys(JSON.parse(req.body.updated_estate)).length >
                      0
                    ) {
                      return trx("estate_page_elements")
                        .where({
  ...
  sqlite
    .transaction((trx) => {
      ...
      return trx("estate_page_images")
        ...
        .select("estate_image_development_url", "estate_image_production_url")
        .then((rows) => {
